...
def setup_generic_relations(model_class):
    ...
    """
    Action = apps.get_model('activity', 'action')

    if Action is None:
        raise RegistrationError(
            'Unable get activity.Action. Potential circular imports '
            'in initialisation. Try moving activity app to come after the '
    ...
    for field in ('actor', 'target', 'action_object'):
        ...
        rel = GenericRelation('activity.Action', **kwargs)
    ...
    def check(self, model_class_or_object):
        ...
        if model_class not in self:
            raise ImproperlyConfigured(
                'The model %s is not registered. Please use apps.activity.registry '
